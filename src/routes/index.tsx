...
import { analyzePage, captureLead } from "@/lib/funnel.functions";
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (isIOS) {
        ...
        if (!opened) toast.info(isEsDownloadHint(lang));
    ...
    } catch (e) { console.error(e); toast.error(t("error_generic")); }
        ...
        <AnimatePresence mode="wait">
          {phase === "input" && (
            <motion.section key="hero" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center">
              <img src={logo} alt="HERO OS" width={120} height={120} className="mx-auto mb-6 h-24 w-24 md:h-28 md:w-28" />
          ...
          {phase === "loading" && (
            <motion.section key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12">
              <h2 className="mb-8 text-center text-xl font-semibold text-foreground">{t("analyzing")}</h2>
            ...
            <motion.section key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10">
              ...
              <div className="overflow-hidden rounded-2xl border-2 border-gold/50 bg-gradient-to-br from-panel-elevated via-panel to-panel-elevated p-6 shadow-2xl shadow-gold/10 md:p-8">
                <div className="grid items-center gap-6 md:grid-cols-[1fr_auto]">
                  <div>
                    <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-gold"><Calendar className="h-3 w-3" />{t("hero_eyebrow")}</div>
                    <h3 className="text-xl font-bold leading-tight text-foreground md:text-2xl">{t("cta_pre_mockup_title")}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground md:text-base">{t("cta_pre_mockup_sub")}</p>
                    <p className="mt-3 text-xs font-medium text-gold/90">{t("cta_pre_mockup_reasons")}</p>
                  </div>
                  <a href={SCHEDULER_URL} target="_blank" rel="noopener noreferrer" className="md:justify-self-end"><Button size="lg" className="h-14 w-full bg-gold px-8 text-base font-bold text-gold-foreground shadow-lg shadow-gold/30 hover:bg-gold/90 md:w-auto"><Calendar className="h-5 w-5" />{t("cta_pre_mockup_btn")}</Button></a>
                </div>
              </div>

              <div className="overflow-hidden rounded-2xl border border-gold/30 bg-panel">
                <div className="flex items-center justify-between border-b border-border/40 px-5 py-3"><h3 className="text-sm font-semibold text-gold uppercase tracking-wider">{t("mockup_title")}</h3></div>
                <div className="relative h-[520px]">
                  {mockupHtml ? (
                    <iframe title="Redesigned mockup" srcDoc={mockupHtml} className="h-full w-full bg-white" sandbox="allow-same-origin" />
                  ) : (
                    <div className="flex h-full items-center justify-center text-muted-foreground"><div className="text-center"><div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent" /><p className="text-sm">{t("step_mockup")}</p></div></div>
                  )}
                  {phase === "report" && <div className="pointer-events-none absolute inset-0 backdrop-blur-md" />}
                  {phase === "report" && (
                    <div className="absolute inset-0 flex items-end bg-gradient-to-t from-background via-background/80 to-transparent p-6">
                      <form onSubmit={handleCapture} className="mx-auto w-full max-w-xl rounded-2xl border border-gold/40 bg-panel-elevated p-6 shadow-2xl">
                        <div className="mb-4 flex items-center gap-2 text-gold"><Lock className="h-4 w-4" /><h4 className="text-base font-bold">{t("unlock_title")}</h4></div>
                        <p className="mb-4 text-xs text-muted-foreground">{t("unlock_sub")}</p>
                        <div className="grid gap-3 sm:grid-cols-2">
                          <Input placeholder={t("first_name")} value={form.first_name} onChange={(e) => setForm({ ...form, first_name: e.target.value })} required maxLength={80} />
                          <Input placeholder={t("last_name")} value={form.last_name} onChange={(e) => setForm({ ...form, last_name: e.target.value })} required maxLength={80} />
                        </div>
                        <Input type="email" placeholder={t("email")} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required maxLength={200} className="mt-3" />
                        <Button type="submit" disabled={submitting} className="mt-4 w-full bg-gold text-gold-foreground hover:bg-gold/90">{submitting ? t("sending") : t("unlock_btn")}</Button>
                      </form>
                    </div>
                  )}
                </div>
              ...
              {phase === "captured" && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <div className="rounded-2xl border border-gold/40 bg-gradient-to-br from-panel to-panel-elevated p-8 text-center">
                    ...
                    <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row"><Button onClick={handleDownload} disabled={downloading} size="lg" variant="outline" className="border-gold/40 text-foreground hover:bg-gold/10"><Download className="h-4 w-4" />{downloading ? "…" : t("download_pdf")}</Button></div>
                  </div>
...
function isEsDownloadHint(lang: "en" | "es") {
  return lang === "es"
    ? "El PDF se abrirá en una nueva pestaña. Usa Compartir → Guardar en Archivos para conservarlo."
    : "The PDF will open in a new tab. Use Share → Save to Files to keep it.";
}
