...
const tone = {
  purple: { border: "border-violet-500/60", glow: "shadow-violet-500/10", text: "text-violet-400", bar: "bg-violet-500", number: "bg-violet-600" },
  orange: { border: "border-orange-500/60", glow: "shadow-orange-500/10", text: "text-orange-400", bar: "bg-orange-500", number: "bg-orange-500" },
  teal: { border: "border-teal-400/60", glow: "shadow-teal-400/10", text: "text-teal-300", bar: "bg-teal-400", number: "bg-teal-500" },
} as const;
  ...
  return (
    <section className="relative mt-20 overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-b from-[#0b1224] via-[#080e1d] to-[#050914] px-4 py-12 shadow-2xl md:px-8 md:py-16">
      ...
      <div className="relative z-10 text-center">
        <div className="mx-auto mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white/60"><span className="text-fuchsia-400">✦</span> HERO OS</div>
        ...
        {c.steps.map((s, i) => {
          ...
          return (
            <motion.article key={s.n} whileHover={{ y: -6 }} className={`relative overflow-hidden rounded-3xl border ${t.border} bg-black/20 p-5 shadow-xl ${t.glow} backdrop-blur-sm`}>
              <div className="flex items-start gap-4"><div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${t.number} text-lg font-black text-white shadow-lg`}>{s.n}</div><div><h3 className={`text-2xl font-black ${t.text}`}>{s.title}</h3><p className="mt-1 text-sm text-white/65">{s.sub}</p></div></div>
      ...
      <div className="relative z-10 mt-5 overflow-hidden rounded-3xl border border-sky-500/60 bg-gradient-to-r from-sky-500/5 via-white/[0.02] to-teal-500/5 p-5 shadow-xl md:p-7">
        <div className="grid items-center gap-7 md:grid-cols-[1fr_1.2fr_1fr]">
          ...
          <div className="text-center md:text-left"><p className="text-lg font-black text-white">{c.download.cta} 🎉</p><p className="mt-2 text-sm text-white/55">{lang === "es" ? "Descarga el reporte y llévalo a tu equipo, o agenda una llamada para que lo construyamos por ti." : "Download the report, share it with your team, or book a call and let us build it for you."}</p><div className="mt-5 flex flex-col gap-3 sm:flex-row md:flex-col"><div className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-400 px-5 py-3 font-bold text-slate-950 shadow-lg shadow-teal-400/20"><Download className="h-4 w-4" />{lang === "es" ? "Descargar PDF" : "Download PDF"}</div><a href="https://calendly.com/jsbusinesscoach/hero-os-strategy-call" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-orange-400/60 px-5 py-3 font-bold text-orange-300 hover:bg-orange-400/10"><span>▣</span>{lang === "es" ? "Agendar llamada gratis" : "Book Free Strategy Call"}</a></div></div>
        </div>
      </div>
