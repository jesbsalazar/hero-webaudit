...
export function HeroLoadingCharacter({ mode, step, language = "en" }: HeroLoadingCharacterProps) {
  ...
  const eyebrow = isAudit
    ? language === "es" ? "MODO AUDITORÍA" : "ROAST MODE"
    : language === "es" ? "MODO CONSTRUCCIÓN" : "BUILD MODE";
  const sub = isAudit
    ? language === "es" ? "Buscando qué está frenando tus conversiones" : "Finding what is holding your conversions back"
    : language === "es" ? "Convirtiendo los hallazgos en una página mejor" : "Turning the findings into a better page";
    ...
    <div className="mx-auto mb-8 w-full max-w-3xl">
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4 px-5 pt-5 md:px-7">
          ...
          <div className={`rounded-full border px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] ${isAudit ? "border-gold/35 bg-gold/10 text-gold" : "border-primary/35 bg-primary/10 text-primary"}`}>
            {eyebrow}
          </div>
        </div>
        ...
        <div className="relative px-5 pb-5 pt-4 md:px-7 md:pb-7">
          <motion.img
            src={image}
            alt={isAudit ? "HERO OS audit" : "HERO OS builder"}
